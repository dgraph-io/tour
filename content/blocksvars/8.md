...
This query can be done in a single block, but the example below shows how values
are available in other blocks when the corresponding UIDs are present in the
value map.
...
UIDs and see what happens. For example, change `avs` to query for a different
