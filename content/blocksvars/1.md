...
loaded the million triple movie dataset. Run `make seed-movie-dataset` from the
tour repository root to do so. On Windows (or if you don't have `make` installed),
use `docker exec tour-seed make seed-movie-dataset` instead. {{% /notice %}}
