...
```
{
  ...
  var(func: eq(name@en, "Comedy")) {
    ~genre {
      starring {
        comedy_actors as performance.actor
      }
    }
  }
  ...
  var(func: eq(name@en, "Horror")) {
    ~genre {
      starring {
        horror_actors as performance.actor
      }
    }
  }

  # Find actors in BOTH sets using @filter
  actors_in_both_genres(func: uid(comedy_actors)) @filter(uid(horror_actors)) {
    name@en
  }
}
```
...
The pattern is:
1. **Query 1**: Collect UIDs into variable `A`
2. **Query 2**: Collect UIDs into variable `B`
3. **Query 3**: Start with `uid(A)` and filter by `uid(B)` to get the intersection
